...
"""
This is a manual running version. For times when the automation fails and I
forget to fix
"""

# CLI argument parser for setting path of files to write to.
parser = argparse.ArgumentParser(description='IP Daily Digest - Manual Run')
parser.add_argument('--path', type=str, default='./', help='Path to project.')
parser.add_argument('--dldir', type=str, default='./', help='Path to downloads.')
parser.add_argument('--dldf', type=str, help='Download file date')
parser.add_argument('--daysback', type=int, help='number of days back')

dir_path = parser.parse_args().path

# Globals
DOWNLOAD_DIR = parser.parse_args().dldir
DOWNLOAD_DATE_FILE = parser.parse_args().dldf
DAYS_BACK = parser.parse_args().daysback
...
def gather_rir_stats():
    ...
    rirs_results['afrinic'] = collect_stats(
        DOWNLOAD_DIR + 'delegated-afrinic-extended-' + DOWNLOAD_DATE_FILE, 4)

    rirs_results['apnic'] = collect_stats(
        DOWNLOAD_DIR + 'delegated-apnic-extended-' + DOWNLOAD_DATE_FILE, 31)

    rirs_results['arin'] = collect_stats(
        DOWNLOAD_DIR + 'delegated-arin-extended-' + DOWNLOAD_DATE_FILE, 4)

    rirs_results['lacnic'] = collect_stats(
        DOWNLOAD_DIR + 'delegated-lacnic-extended-' + DOWNLOAD_DATE_FILE, 4)

    rirs_results['ripe'] = collect_stats(
        DOWNLOAD_DIR + 'delegated-ripencc-extended-' + DOWNLOAD_DATE_FILE, 4)

    return rirs_results

def collect_stats(url, start_pos):
    with open(url) as f:
        delegated_lines = f.read().splitlines()
...
def markdown_summed_report(report):
    ...
    now = datetime.datetime.now() - datetime.timedelta(days=DAYS_BACK)
    markdown = ""
    markdown += "```\n"
...
def get_previous_report(filename):
    with open(filename, 'r+') as f:
        ...
        yesterday = str(datetime.date.today() - datetime.timedelta(days=days_back))
...
generate_graphs(DAYS_BACK, 'ipv4', 'global', dir_path, 'ipv4-figure.png')
generate_graphs(DAYS_BACK, 'ipv6', 'global', dir_path, 'ipv6-figure.png')
write_daily_digest(
    dir_path,
    'README.md',
    global_results,
...
generate_graphs(DAYS_BACK, 'ipv4', 'afrinic', dir_path + 'archives/AFRINIC/', 'ipv4-figure.png')
generate_graphs(DAYS_BACK, 'ipv6', 'afrinic', dir_path + 'archives/AFRINIC/', 'ipv6-figure.png')
write_daily_digest(
...
generate_graphs(DAYS_BACK, 'ipv4', 'apnic', dir_path + 'archives/APNIC/', 'ipv4-figure.png')
generate_graphs(DAYS_BACK, 'ipv6', 'apnic', dir_path + 'archives/APNIC/', 'ipv6-figure.png')
write_daily_digest(
...
generate_graphs(DAYS_BACK, 'ipv4', 'arin', dir_path + 'archives/ARIN/', 'ipv4-figure.png')
generate_graphs(DAYS_BACK, 'ipv6', 'arin', dir_path + 'archives/ARIN/', 'ipv6-figure.png')
write_daily_digest(
...
generate_graphs(DAYS_BACK, 'ipv4', 'lacnic', dir_path + 'archives/LACNIC/', 'ipv4-figure.png')
generate_graphs(DAYS_BACK, 'ipv6', 'lacnic', dir_path + 'archives/LACNIC/', 'ipv6-figure.png')
write_daily_digest(
...
generate_graphs(DAYS_BACK, 'ipv4', 'ripencc', dir_path + 'archives/RIPE_NCC/', 'ipv4-figure.png')
generate_graphs(DAYS_BACK, 'ipv6', 'ripencc', dir_path + 'archives/RIPE_NCC/', 'ipv6-figure.png')
write_daily_digest(
...
year = DOWNLOAD_DATE_FILE[:4]
month = DOWNLOAD_DATE_FILE[4:6]
day = DOWNLOAD_DATE_FILE[6:8]
proc = subprocess.Popen(
    ["/usr/bin/git", "commit", '--amend', '--date="' + year + '-' + month + '-' + day + ' 10:00:00"', '-m "Added new IP daily digest"'],
    stdout=subprocess.PIPE
)
print(proc.stdout.read())
